...
import {
  ...
  placeFienzaHoles,
  updateFienzaDropdown // ✅ needed for rebuild
} from './fienza.js';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function isDoubleModeActive() {
    // Adjust the ID to match your actual checkbox element
  ...
  function updateCornerRadiusMax() {
    const shape = basinShape?.value;   // ✅ use basinShape, not basinShapeSelect
    let maxRadius = 250;
  ...
  if (basinShape && cornerRadiusFields && cornerRadiusSlider && cornerRadiusValue) {
    basinShape.addEventListener('change', () => {
      ...
      if (shape === 'square' || shape === 'rect') {
        ...
        updateCornerRadiusMax(); // recalc max when shape changes
      } else {
        cornerRadiusFields.style.display = 'none';
        BenchtopCanvas.setCornerRadius(0);
        updateCornerRadius(0); // reset UI fields too
      }
      BenchtopCanvas.draw();
      App.updateReport();
    });
  ...
  function updateDoubleVisibility() {
    ...
    // Show/hide Tap2/Waste2 buttons based on checkbox state
...
function updateAndPlaceFienza() {
  ...
  // ✅ Force single mode if checkbox is OFF
  if (match && !isDouble) {
    match = { ...match, double: "N" };
  }

  // ✅ Pass double flag and size to filtering
  ...
  // ==============================
// Placement button wiring (uses canvas.js setter)
// ==============================
placeTapBtn.addEventListener('click', () => {
  // Always use T1 as the primary tap hole
...
placeWasteBtn.addEventListener('click', () => {
  // Always use W1 as the primary waste hole
...
placeTap2Btn.addEventListener('click', () => {
  // Secondary tap hole only in double mode
...
placeWaste2Btn.addEventListener('click', () => {
  // Secondary waste hole only in double mode
  ...
  doubleModeCheckbox.addEventListener('change', (e) => {
    if (!e.target.checked) {
      ...
      // Double turned ON → show second-hole fields
      const tap2Fields = document.getElementById('tap2Fields');
      const waste2Fields = document.getElementById('waste2Fields');
...
tapPosition.addEventListener('change', () => {
  ...
  sizeSelect.addEventListener('change', () => {
    ...
    updateDoubleVisibility();
    updateAndPlaceFienza(); // respects checkbox state
  });

  // ==============================
  // Double toggle behaviour
  // ==============================
...
basinShape.addEventListener('change', e => {
  ...
  basinSizeInputs.style.display = e.target.value ? 'block' : 'none'; // 🔧 show/hide size fields
  ...
  // Re‑attach listeners for whichever fields were injected
  ['basinDiameter', 'basinSide', 'basinWidth', 'basinHeight'].forEach(id => {
    const el = document.getElementById(id);
    if (el) {
      el.addEventListener('input', () => {
        App.applyBasin();          // update basin size
        updateCornerRadiusMax();   // recalc radius clamp
      });
    }
  });
  ...
  // ==============================
  // Inputs live updates (single holes only)
  // ==============================
...
// ==============================
// Placement button toggle logic
// ==============================
function updatePlacementButtons() {
  // Tap1 (single or primary in double)
  ...
  // Waste1 (single or primary in double)
  ...
  window.App.applyBasin = function () {
    ...
    // 🔧 Sync Basin X/Y fields immediately after placement
...
updateCornerRadiusMax();
  ...
  App.updateReport();
  updatePlacementButtons();
}); // close DOMContentLoaded only once
